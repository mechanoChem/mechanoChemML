...
from mechanoChemML.workflows.pde_solver.pde_workflow_steady_state_heter_v2 import PDEWorkflowSteadyState
...
class LayerDiffusionSteadyStateBulkResidual(pde_layers.LayerBulkResidual):
    ...
    def __init__(self, dh, normalization_factor=2.0, name='R_bulk_diffusion'):
    ...
    def call(self, input):
        ...
        data = self.GetElementInfo(input[0])
        self.D0 = input[1]
        # print('D0 shape: ', tf.shape(self.D0))
...
class WeakPDESteadyStateDiffusion(PDEWorkflowSteadyState):
    ...
    def _bulk_residual(self, y_pred, pde_parameters):
        ...
        """
        # print('pde_parameter shape: ', tf.shape(pde_parameters))
        elem_bulk_residual=LayerDiffusionSteadyStateBulkResidual(dh=self.dh)([y_pred, pde_parameters])
