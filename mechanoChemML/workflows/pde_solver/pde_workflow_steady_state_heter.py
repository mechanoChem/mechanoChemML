...
from mechanoChemML.src.nn_models import BNN_user_weak_pde_general_heter
import mechanoChemML.src.pde_layers as pde_layers
from mechanoChemML.workflows.pde_solver.pde_utility import plot_PDE_solutions, plot_fields, split_data_heter, ExpandDatasetHeter, exe_cmd, BatchDataHeter, plot_one_field_hist, plot_one_field_stat, plot_one_field,plot_PDE_solutions_new
...
class PDEWorkflowSteadyState:
    ...
    def _load_data(self, only_neumann_data=False, test_folder=''):
        ...
        for one_folder in data_folder_list:
            ...
            for f1 in file_list:
                ...
                mat_path = f1.replace('features', 'mats')
                one_mat = np.load(mat_path)
        ...
        # R_red, y_pred, y_true_dummy, _, _, _ = self._compute_residual(the_feature, pde_parameters, self.labels)
        ...
        if only_testing:
            ...
            self.test_seq = BatchDataHeter(data=(self.test_dataset, self.mats, self.test_label), batch_size=1)
            self.train_seq = self.test_seq
        else:
            the_feature, the_mat, the_label = ExpandDatasetHeter(self.features, self.mats, self.labels, times=self.expand_times)
            # print(np.shape(the_feature), np.shape(the_mat), np.shape(the_label))
            # print('stop at line 324')
            self.train_dataset, self.train_mat, self.train_label, self.val_dataset, self.val_mat, self.val_label, self.test_dataset, self.test_mat, self.test_label = split_data_heter(the_feature, the_mat, the_label, self.batch_size, split_ratio=['0.8', '0.1', '0.1'])
            ...
            self.train_seq = BatchDataHeter(data=(self.train_dataset, self.train_mat, self.train_label), batch_size=self.batch_size)
            self.val_seq   = BatchDataHeter(data=(self.val_dataset,   self.val_mat,   self.val_label),   batch_size=self.batch_size)
            self.test_seq  = BatchDataHeter(data=(self.test_dataset,  self.test_mat,  self.test_label),  batch_size=self.batch_size)
    ...
    def _compute_residual(self, features, pde_parameters, y_pred, only_y_pred=False):
        """
        ...
        elem_bulk_residual=self._bulk_residual(y_pred, pde_parameters)
    ...
    def _loss_probabilistic(self):
        ...
        def loss(y_true, y_pred):
            pde_parameters = self.model.get_pde_parameters()
            # print('pde parameters: ', pde_parameters)
            ...
            R_red, y_pred, y_true_dummy, _, _, _ = self._compute_residual(inputs, pde_parameters, y_pred)
    ...
    def _load_saved_cnn_model(self):
        ...
        # print('saved_kernel:', saved_kernel)
        # print('saved_bias:', saved_bias)
        ...
        for v0 in self.model.trainable_variables :
            ...
            if v0.name.find('kernel_posterior_loc') >= 0 :
                ...
                # print('v0= ', type(v0), v0.name, np.shape(v0_value), kernel_ind)
    ...
    def _build_model(self):
        ...
        self.model = BNN_user_weak_pde_general_heter(
                layers_str=self.config['NN']['NNArchitecture'],
                NUM_TRAIN_EXAMPLES=len(self.train_seq), # total batch numbers
                Sigma2=self.Sigma2)
        self.optimizer = self._build_optimizer()
    ...
    def _train(self):
        ...
        #-------------------- not sure if impact BNN warm start or not---
        self.model.predict_on_batch(self.test_seq[0][0])
        ...
        # redefined for customized_trainer loss
        def my_loss(y_true, y_pred):
            print("Not updated for heter structure, exit...")
            exit(0)
            pde_parameters = self.model.get_pde_parameters()
            print('pde parameters: ', pde_parameters)
        ...
        for epoch in range(self.epochs):
            ...
            if self.val_seq is not None:
                for step, (batch_x, batch_y) in enumerate(self.val_seq):
                    ...
                    if self.UseTwoNeumannChannel :
                        ...
                        inputs = y_pred[:,:,:,self.dof:3*self.dof] # Old Neumann Channel
                    y_pred = y_pred[:,:,:,0:self.dof]
                    R_red, y_pred, y_true_dummy, R_body, dR_neumann, _ = self._compute_residual(inputs, pde_parameters, y_pred)
                    ...
                    # print(type(res_body), type(self.BetaPDELoss), type(R_body))
            ...
            # print(type(epoch_mse_loss), type(epoch_res_body), type(epoch_res_neu))
            # print(epoch_mse_loss, epoch_res_body, epoch_res_neu)
    ...
    def debug_problem(self, use_label=False):
        ...
        try: 
            prediction = self.model.predict([self.features, self.mats])
            pde_parameters = self.model.get_pde_parameters()
    ...
    def test(self, test_folder='', plot_png=True, output_reaction_force=False):
        ...
        for _ in range(self.monte_carlo_num):
            ...
            if output_reaction_force:
                _, y_pred, _, _, _, R_fix = self._compute_residual(inputs, pde_parameters, y_pred)
                reaction_force.append(R_fix)
            else:
                y_pred= self._compute_residual(inputs, pde_parameters, y_pred, only_y_pred=True)
    ...
    def test_residual_gaussian(self, noise_std=1.e-3, sample_num=10000):
        ...
        self.mats = tf.convert_to_tensor(self.mats, dtype=tf.float32)
        ...
        mats     = tf.tile(self.mats,     [sample_num,1])
        print('mats (pde parameters) are not tested! exit...')
        exit(0)
        y_pred = labels
        ...
        for s0 in dummy_seq:
            ...
            R, _, _, _, _, _ = self._compute_residual(features[start:end,:,:,:], mats[start:end,:], y_pred[start:end,:,:,:])
            # print(tf.shape(R))
            R_list.append(R)
        R = np.vstack(R_list)
        # print(tf.shape(R))
