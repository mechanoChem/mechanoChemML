...
def plot_field_results(list_of_field, list_of_field_name, dof, dof_name,  filename='', print_data=False, vmin=None, vmax=None, Tex=False):
    ...
    for i0 in range(0, dof):
        ...
        for j0 in range(1, tot_img+1):
            ...
            if _field_name.find('DNS') >= 0 or  _field_name.find('Mean') >= 0 or _field_name.find('Std') >= 0 or _field_name.find('Sol') >= 0:
                if x_direction:
        ...
        if x_direction:
            ...
            plt.plot(coor, (label_field[:, loc]-0.5)*2.0, 'k')
            plt.plot(coor, (det_field[:, loc]-0.5)*2.0, 'b')
            plt.plot(coor, (mean_field[:, loc]-0.5)*2.0, 'k')
            plt.fill_between(coor, (mean_field[:, loc]-0.5)*2.0 - 2.0*2.0*std_field[:, loc], (mean_field[:, loc]-0.5)*2.0 + 2.0*2.0*std_field[:,loc], alpha=0.7, facecolor='gray')
...
def plot_each_bvp_results_with_quantitative_line(ra=0):
    """ """
    # cnn
    saved_data = "results/2022-03-12T05-01-exp-3-57-NN-cnn-x11-B1024-E20000-I100-mc1-1S0.0e+00-2S0.0e+00-Nadam-2.5e-04-..data-20220312050122.pickle"
    ...
    # # bnn
    # lr = 1e-8
    saved_data = "results/2022-03-19T20-08-exp-3-60-BNN-bnn-5-x11-B256-E20000-I100-mc100-1S2.0e-04-2S1.0e-03-Nadam-2.0e-06-..data-20220319200844.pickle"
    # # lr = 1e-6
...
def plot_bvp_results_with_quantitative_line(pixel=32):
    ...
    if pixel == 32:
        ...
        saved_data = "results/2022-03-12T05-01-exp-2-58-NN-cnn-x14-B512-E20000-I50-mc1-1S0.0e+00-2S0.0e+00-Nadam-2.5e-04-....data-20220312050122.pickle"
    ...
    if pixel == 32:
        ...
        saved_data = "results/2022-03-19T02-35-exp-1-60-BNN-bnn-6-x14-B256-E20000-I100-mc100-1S1.0e-04-2S1.0e-03-Nadam-2.0e-06-....data-20220319023529.pickle"
    sys.argv[3] = saved_data
    problem = thisPDESystem()
    # problem.run()
    _inputs, _labels, _mean, _var, _std = problem.test(test_folder='DNS', plot_png=False)
