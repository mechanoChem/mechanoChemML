...
def plot_bvp_results_with_quantitative_line(pixel=32):
    ...
    # if pixel == 32:
        # # cnn
        # # saved_data = "results/2022-03-19T12-17-exp-13-60-BNN-bnn-4-x14-B256-E20000-I100-mc100-1S1.0e-03-2S1.0e-03-Nadam-1.0e-06-..data-20220319121753.pickle"
    ...
    if pixel == 32:
        # cnn
        # saved_data = "results/2022-03-19T12-17-exp-13-60-BNN-bnn-4-x14-B256-E20000-I100-mc100-1S1.0e-03-2S1.0e-03-Nadam-1.0e-06-..data-20220319121753.pickle"
        saved_data = "results/2022-03-19T02-24-exp-4-58-BNN-bnn-3-x14-B256-E20000-I100-mc100-1S1.0e-03-2S1.0e-03-Nadam-2.0e-06-..data-20220319022440.pickle"
    elif pixel == 64:
        saved_data = None

    sys.argv.extend(['bnn.ini', '-rf', saved_data, '-ra', '19999'])
    # sys.argv[3] = saved_data
    problem = thisPDESystem()
    # problem.run()
    _inputs, _labels, _mean, _var, _std = problem.test(test_folder='DNS', plot_png=False)
...
def load_octagon_dns():
    import PIL.Image
    img_file = 'postprocess/octagon-dns-1-dirichelet.png'
