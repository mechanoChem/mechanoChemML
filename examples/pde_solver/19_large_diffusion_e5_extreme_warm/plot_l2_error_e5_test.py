...
summary_pentagon = summary[summary['e5-extreme-all-mean'].notna()]
summary_e456 = summary[summary['e4-32k-all-mean'].notna()].reset_index()
...
for i0 in range(len(summary_pentagon)):
    ...
    plt.text(0.75, -0.075, "train", fontsize=20)
    plt.text(2.3, -0.075, "test", fontsize=20)
    # plt.savefig('l2_error_pentagon_'+str(i0)+'.png', bbox_inches='tight', dpi=600)
    plt.savefig('l2_error_pentagon_'+str(i0)+'_cnn_scale_max_physical.png', bbox_inches='tight', dpi=600)
    # plt.show()
    # exit(0)
...
# for i0 in range(len(summary_e456)):
    ...
    # plt.ylim([0,0.25])
    # plt.xticks(loc, tick)
    # plt.text(1.1, -0.035, "train", fontsize=20)
    # plt.text(7.25, -0.035, "test", fontsize=20)
    # plt.axvline(x=3.5, ls='--',color='k')
    # if np.isnan(summary_e456['e9-96k-all-mean'   ][i0]):
        # e456 += 1
        # plt.savefig('l2_error_e456_'+str(e456)+'.png', bbox_inches='tight')
    # else :
        # e4569 += 1
        # plt.savefig('l2_error_e4569_'+str(e4569)+'.png', bbox_inches='tight')
    # # plt.show()
