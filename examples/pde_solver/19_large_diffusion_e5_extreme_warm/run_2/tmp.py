...
def statistics_data(_inputs, _labels, _mean, _var, _std, _edge):
    ...
    _labels_max = (_labels_max - 0.5) * 2.0
    # print(_labels_max)

    # dy_DNS_NN = _labels - _mean
    dy_DNS_NN = tf.divide(( (_labels - 0.5)*2.0 - (_mean-0.5)*2.0), _labels_max) # 
...
if __name__ == '__main__':
    ...
    # _inputs, _labels, _mean, _var, _std = problem.test(test_folder='new_test/extreme/', plot_png=False)
    _inputs, _labels, _mean, _var, _std = problem.test(test_folder='new_test/extreme/', plot_png=True)
    text_entries = statistics_data(_inputs, _labels, _mean, _var, _std, _edge=5)
    text_output.extend(text_entries)
    # print('text_ouput:', text_output)
