...
def plot_each_bvp_results_with_quantitative_line(ra=0):
    """ """
    # cnn
    # saved_data = "results/2020-12-17T08-20-NN-linear-30-bvp-final-1-cnn-x9-B128-E20000-I100-mc1-1S0.0e+00-2S0.0e+00-Nadam-2.5e-04-data16x16-linear-DNS-30-E30-Nu0.3-20201217082006.pickle"
    saved_data = "results/2022-03-12T05-01-exp-9-60-NN-cnn-x9-B256-E20000-I50-mc1-1S0.0e+00-2S0.0e+00-Nadam-2.5e-04-..data-20220312050122.pickle"
    ...
    # # bnn
    # saved_data = "results/2020-12-30T15-50-BNN-linear-30-bvp-final-1-bnn-x9-B64-E5000-I50-mc50-1S1.0e-04-2S1.0e-04-Nadam-1.0e-08-data16x16-linear-DNS-30-E30-Nu0.3-20201230155031.pickle"
    # saved_data = "results/2021-01-04T07-35-BNN-linear-30-bvp-final-1-bnn-x9-B64-E100-I50-mc50-1S1.0e-04-2S1.0e-04-Nadam-1.0e-08-data16x16-linear-DNS-30-E30-Nu0.3-20210104073548.pickle"
    # saved_data = "results/2022-03-12T05-01-exp-9-60-NN-cnn-x9-B256-E20000-I50-mc1-1S0.0e+00-2S0.0e+00-Nadam-2.5e-04-..data-20220312050122.pickle"
    saved_data = "results/2022-06-13T08-33-exp-6-59-BNN-bnn-x9-B256-E20000-I100-mc100-1S1.0e-05-2S1.0e-03-Nadam-2.0e-06-..data-20220613083321.pickle"
    # ra = 100
    ...
    # for i0 in range(0, np.shape(_inputs)[0]):
        # plot_fields(
                ...
                # dof = 2, 
                # dof_name = ['(X)', '(Y)'],
                # filename = 'linear-input-dns-'+str(i0) + restart_name +'.png',
                # Tex = True,
                # vmin = -2,
                # vmax = 0.5,
                # fig_size = 4.0,
    ...
    for i0 in range(0, np.shape(_inputs)[0]):
        plot_field_results(
                ...
                dof = 2, 
                dof_name = ['(X)', '(Y)'],
                filename = 'linear-results-dns-'+str(i0) + restart_name +'.png',
                Tex = True,
                )
...
def output_inter_extra_with_reaction_force(sim_id=0):
    ...
    if sim_id == 0:
        ...
        sys.argv.extend(['l-shape-32-bnn-new.ini', '-rf', saved_data])
    else:
        if sim_id == 1:
...
def plot_lshape_bvp_results_with_quantitative_line():
    ...
    saved_data = "results/2021-01-05T00-33-NN-l-shape-32-adam-2-x10-B256-E50000-I100-mc1-1S0.0e+00-2S0.0e+00-Adam-2.5e-04-datal-shape-32x32-correct-20210105003316.pickle"
    sys.argv.extend(['l-shape-32-adam-2.ini', '-rf', saved_data])
...
def plot_field_results_lshape(list_of_field, list_of_field_name, dof, dof_name,  filename='', print_data=False, vmin=None, vmax=None, Tex=False):
    ...
    for i0 in range(0, dof):
        ...
        if i0 == 0:
            ...
            plt.plot(coor_flip, label_field[:, loc], 'r')
            # plt.plot(x_label['Points:1'],x_label['ResultX'], 'r')
            plt.plot(coor_flip, mean_field[:, loc], 'k')
            plt.fill_between(coor_flip, mean_field[:, loc] - 2*std_field[:, loc], mean_field[:, loc] + 2*std_field[:,loc], alpha=0.7, facecolor='gray')
            plt.xlabel(label_text)
            plt.ylabel('value')
            plt.legend(['DNS', 'Mean'])
            plt.xlim([0,1.0])
