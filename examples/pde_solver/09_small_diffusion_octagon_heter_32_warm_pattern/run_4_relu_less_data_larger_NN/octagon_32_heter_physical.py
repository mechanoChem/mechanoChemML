    ...
    for i0 in range(0, dof):
        ...
        for j0 in range(1, tot_img+1):
            ...
            if _field_name.find('Sol') >= 0 or  _field_name.find('Mean') >= 0:
                ...
                c_img = plt.imshow(2.0*masked_field, cmap=hot)  # tensor
        ...
        if index_i0 == 0:
            x_label = pd.read_csv('postprocess/h-0.515-3.0.csv')  
            y_label = pd.read_csv('postprocess/v-0.485-3.0.csv')  
        if index_i0 == 1:
            x_label = pd.read_csv('postprocess/h-0.515-4.0.csv')  
            y_label = pd.read_csv('postprocess/v-0.485-4.0.csv')  
        if index_i0 == 2:
            x_label = pd.read_csv('postprocess/h-0.515-5.0.csv')  
            y_label = pd.read_csv('postprocess/v-0.485-5.0.csv')  


        # plot quantitative data
        fig = plt.figure(figsize=(fig_size+1, fig_size))
        # plt.plot(coor, label_field[loc, :], 'r')
        # plt.plot(x_label['Points:0'],(x_label['Result'] - 0.5)*2.0, 'k')
        plt.plot(x_label['Points:0'],x_label['concentration'] , 'k')
        ...
        # plt.plot(coor, label_field[:, loc], 'r')
        # plt.plot(y_label['Points:1'],(y_label['Result']-0.5)*2.0, 'k')
        plt.plot(y_label['Points:1'],y_label['concentration'], 'k')
        plt.plot(coor, (det_field[:, loc]-0.5)*2.0, 'b')
        plt.plot(coor, (mean_field[:, loc]-0.5)*2.0, 'r')
        plt.fill_between(coor, (mean_field[:, loc]-0.5)*2.0 - 2.0*2.0*std_field[:, loc], (mean_field[:, loc]-0.5)*2.0 + 2.0*2.0*std_field[:,loc], alpha=0.7, facecolor='gray')
...
def plot_bvp_results_with_quantitative_line(pixel=32):
    ...
    if pixel == 32:
        ...
        saved_data = "results/2022-07-06T14-34-exp-14-60-NN-cnn-x12-B1024-E30000-I100-mc1-1S0.0e+00-2S0.0e+00-Nadam-2.5e-04-data-20220706143450.pickle"
    elif pixel == 64:
        saved_data = None
    ...
    if pixel == 32:
        ...
        saved_data = "results/2022-07-07T07-07-exp-8-59-BNN-bnn-x12-B256-E20000-I100-mc100-1S1.0e-05-2S1.0e-03-Nadam-2.0e-06-data-20220707070725.pickle"
    elif pixel == 64:
        saved_data = None
...
def load_octagon_dns():
    ...
    if index_i0 == 0:
        img_file = 'postprocess/dns-3.0.png'
    if index_i0 == 1:
        img_file = 'postprocess/dns-4.0.png'
    if index_i0 == 2:
        img_file = 'postprocess/dns-5.0.png'
